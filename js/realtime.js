...
class EcoQuestRealtime {
    ...
    showAchievementModal(achievement) {
        ...
        if (!document.querySelector('#achievement-modal-styles')) {
            const styles = document.createElement('style');
            styles.id = 'achievement-modal-styles';
            styles.textContent = `
                @keyframes fadeIn {
                    from { opacity: 0; }
                    to { opacity: 1; }
            ...
            <div class="leaderboard-list">
                ${leaderboardData.map((user, index) => `
                    <div class="leaderboard-item ${index < 3 ? 'top-three' : ''}" data-rank="${index + 1}">
                        <div class="rank">
                            ${index < 3 ? 
                                `<i class="fas fa-trophy" style="color: ${['#FFD700', '#C0C0C0', '#CD7F32'][index]};"></i>` : 
                                `#${index + 1}`
                            }
                        </div>
                        <div class="user-info">
    ...
    // Method to manually trigger user data refresh
    refreshUserData() {
        this.send({ type: 'get_user_data' });
    }
    
    // Method to join/leave rooms for specific features
    joinRoom(roomName) {
        this.send({ type: 'join_room', room: roomName });
    }
    
    leaveRoom(roomName) {
        this.send({ type: 'leave_room', room: roomName });
    }
}
